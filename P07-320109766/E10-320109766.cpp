...
float guardarTXT = 0;   // bandera para guardar solo una vez
float habilitarTXT = 0; 
...
bool saveKeyFrames(const char* filename)
{
	...
	for (int i = 0; i < FrameIndex; i++)
	{
		...
		// Imprimir en consola
		printf(
			"Frame %d: Posicion x = %.3f | Posicion y = %.3f | Rotacion eje y = %.3f\n",
			i,
...
void saveFrame(void) //tecla L
{
	...
	/*if (saveKeyFrames("keyframes.txt")) {
		printf("Se guardo \n");
	}*/
}
...
bool loadKeyFrames(const char* filename)
{
	...
	FrameIndex = 0;   // reiniciamos contador
	...
	while (file >> x >> y >> giro)
	{
		...
		// Inicializar los incrementos en 0 (se calcularán después)
		KeyFrame[FrameIndex].movAvion_xInc = 0.0f;
		KeyFrame[FrameIndex].movAvion_yInc = 0.0f;
		KeyFrame[FrameIndex].giroAvionInc = 0.0f;
	...
	file.close();
	std::cout << "Se cargaron " << FrameIndex << " keyframes\n";
	return true;
}
...
void inputKeyframes(bool* keys)
{
	...
	if (keys[GLFW_KEY_7])
	{
		if (guardarTXT < 1)
		{
			...
			printf("Presiona 8 para poder guardar de nuevo\n");
			guardarTXT++;
			habilitarTXT = 0;
		}
	}
	if (keys[GLFW_KEY_8])
	{
		if (habilitarTXT < 1)
		{
			guardarTXT = 0;
			habilitarTXT++;
			printf("Ya puedes volver a guardar presionando 7\n");
		}
	}
}
